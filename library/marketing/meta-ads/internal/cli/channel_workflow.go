...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  meta-ads-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  meta-ads-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "campaigns", "targeting"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  meta-ads-pp-cli workflow status

  # Show status as JSON
  meta-ads-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("meta-ads-pp-cli")
			}
