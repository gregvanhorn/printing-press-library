...
func resolveRead(c *client.Client, flags *rootFlags, resourceType string, isList bool, path string, params map[string]string) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if localErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'meta-ads-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func resolveLocal(resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	...
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'meta-ads-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'meta-ads-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'meta-ads-pp-cli sync' first", resourceType)
		}
	...
	if item == nil {
		return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'meta-ads-pp-cli sync' first", resourceType, id)
	}
