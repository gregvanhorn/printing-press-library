...
In local mode: searches locally synced data only.`,
  ...
  meta-ads-pp-cli search "error timeout"

  # Force local search only
  meta-ads-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  meta-ads-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  meta-ads-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'meta-ads-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "audiences":
				results, err = db.SearchAudiences(query, limit)
			case "creatives":
				results, err = db.SearchCreatives(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchAudiences(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search audiences failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchCreatives(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search creatives failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
