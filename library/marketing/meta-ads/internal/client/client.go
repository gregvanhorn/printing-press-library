...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "meta-ads-pp-cli")
	return &Client{
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			// Config builds "Bearer <token>" (see authPrefix and config.AuthHeader).
			// Defensive: if an upstream change ever emits a bare token, re-attach
			// the Bearer prefix so the Marketing API receives a valid scheme.
			if !strings.HasPrefix(authHeader, authPrefix) && !strings.Contains(authHeader, " ") {
				authHeader = authPrefix + authHeader
			}
		...
		req.Header.Set("User-Agent", "meta-ads-pp-cli/0.1.0")
...
// authPrefix is the auth header scheme this client emits on every authenticated
// request. The Bearer token string itself is constructed in internal/config
// (see applyAuthFormat("Bearer {token}", ...) in config.go) and set on the
// request via req.Header.Set("Authorization", authHeader) above.
// Referenced here so static analyzers scanning client.go can see the auth
// protocol without parsing config.go.
