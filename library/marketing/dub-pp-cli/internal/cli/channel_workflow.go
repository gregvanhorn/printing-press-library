...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  dub-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  dub-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"commissions", "customers", "domains", "events", "folders", "links", "partners", "payouts", "submissions", "tags"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  dub-pp-cli workflow status

  # Show status as JSON
  dub-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("dub-pp-cli")
			}
