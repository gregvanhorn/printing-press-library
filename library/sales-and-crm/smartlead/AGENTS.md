# Smartlead Printed CLI Agent Guide

This directory is a generated `smartlead-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
smartlead-pp-cli doctor --json
smartlead-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
smartlead-pp-cli which "<capability>" --json
...
```bash
smartlead-pp-cli <command> --agent
```
...
```bash
smartlead-pp-cli <command> --help
smartlead-pp-cli <command> --dry-run --agent
```
