...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "smartlead-pp-cli",
		Short: `Smartlead CLI — Every Smartlead feature, plus offline sync, FTS5 search, auto-chunking, and cross-campaign aggregations no other Smartl…`,
		Long: `Smartlead CLI — Every Smartlead feature, plus offline sync, FTS5 search, auto-chunking, and cross-campaign aggregations no other Smartl…
...
Health check: run 'smartlead-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("smartlead-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'smartlead-pp-cli profile list')")
	...
	rootCmd.AddCommand(newEmailAccountsCmd(flags))
	rootCmd.AddCommand(newLeadListCmd(flags))
	rootCmd.AddCommand(newLeadsCmd(flags))
	rootCmd.AddCommand(newMasterInboxCmd(flags))
	rootCmd.AddCommand(newSmartSendersCmd(flags))
	rootCmd.AddCommand(newSmartleadAnalyticsCmd(flags))
	rootCmd.AddCommand(newSpamTestCmd(flags))
	...
	rootCmd.AddCommand(newDownloadStatisticsPromotedCmd(flags))
	rootCmd.AddCommand(newEmailCampaignsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Transcendence commands (cross-campaign aggregations)
	rootCmd.AddCommand(newStaleCmd(flags))
	rootCmd.AddCommand(newOverlapCmd(flags))
	rootCmd.AddCommand(newMailboxBurnoutCmd(flags))
	rootCmd.AddCommand(newReplyVelocityCmd(flags))
	rootCmd.AddCommand(newDelivDriftCmd(flags))
	rootCmd.AddCommand(newReachBudgetCmd(flags))
	rootCmd.AddCommand(newRollUpCmd(flags))
	rootCmd.AddCommand(newReplyClassifyCmd(flags))
