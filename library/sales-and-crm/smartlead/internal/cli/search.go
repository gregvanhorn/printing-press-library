...
In local mode: searches locally synced data only.`,
  ...
  smartlead-pp-cli search "error timeout"

  # Force local search only
  smartlead-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  smartlead-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  smartlead-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /smart-senders/search-domain
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/smart-senders/search-domain", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'smartlead-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "master_inbox":
				results, err = db.SearchMasterInbox(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchMasterInbox(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search master_inbox failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
