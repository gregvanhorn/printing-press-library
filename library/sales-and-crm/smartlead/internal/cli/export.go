...
large datasets as it has no memory pressure.`,
  ...
  smartlead-pp-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  smartlead-pp-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  smartlead-pp-cli export <resource> --format jsonl | jq '.id'`,
		Args: cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			validResources := map[string]bool{
				"download-statistics": true,
				"email-accounts": true,
				"leads": true,
				"webhooks": true,
			}
