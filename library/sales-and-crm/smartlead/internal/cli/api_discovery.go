...
Run 'api <interface>' to see that interface's methods.`,
  ...
  smartlead-pp-cli api

  # Show methods for a specific interface
  smartlead-pp-cli api <interface-name>`,
			...
			if len(args) > 0 {
				...
				for _, child := range root.Commands() {
					if child.Hidden && strings.ToLower(child.Name()) == target {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "\nUse '%s-pp-cli %s <method> --help' for details.\n", "smartlead", child.Name())
						return nil
					}
				}
				return fmt.Errorf("interface %q not found. Run '%s-pp-cli api' to list all interfaces", args[0], "smartlead")
			}
