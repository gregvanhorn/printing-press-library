...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".smartlead-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SMARTLEAD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SMARTLEAD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "smartlead-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.smartlead-pp-cli/feedback.jsonl.
When ` + "`SMARTLEAD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SMARTLEAD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  smartlead-pp-cli feedback list
  smartlead-pp-cli feedback list --limit 5
  smartlead-pp-cli feedback list --json`,
