...
type Config struct {
	...
	Path           string `toml:"-"`
	SmartleadApiKey string `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SMARTLEAD_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "smartlead-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SMARTLEAD_API_KEY"); v != "" {
		cfg.SmartleadApiKey = v
		cfg.AuthSource = "env:SMARTLEAD_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SmartleadApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SMARTLEAD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.SmartleadApiKey
	if token == "" {
		return ""
	}
	if c.SmartleadApiKey == "" {
		return ""
	}
	return token
}
