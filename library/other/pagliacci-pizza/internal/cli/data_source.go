...
func writeThroughCache(resourceType string, data json.RawMessage) {
	db, err := store.Open(defaultDBPath("pagliacci-pizza-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
...
func resolveLocal(resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	...
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'pagliacci-pizza-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'pagliacci-pizza-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'pagliacci-pizza-pp-cli sync' first", resourceType)
		}
	...
	if item == nil {
		return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'pagliacci-pizza-pp-cli sync' first", resourceType, id)
	}
