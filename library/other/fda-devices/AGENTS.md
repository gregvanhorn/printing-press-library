# FDA Devices Printed CLI Agent Guide

This directory is a generated `fda-devices-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
fda-devices-pp-cli doctor --json
fda-devices-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
fda-devices-pp-cli which "<capability>" --json
fda-devices-pp-cli <command> --help
```
...
```bash
fda-devices-pp-cli <command> --agent
```
...
```bash
fda-devices-pp-cli <command> --help
fda-devices-pp-cli <command> --dry-run --agent
```
