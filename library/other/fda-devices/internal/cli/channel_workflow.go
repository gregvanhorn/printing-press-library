...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  fda-devices-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  fda-devices-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"device", "device-510k-json", "device-classification-json", "device-event-json", "device-recall-json", "device-registrationlisting-json",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  fda-devices-pp-cli workflow status

  # Show status as JSON
  fda-devices-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("fda-devices-pp-cli")
			}
