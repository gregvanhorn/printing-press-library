...
Exit codes & warnings:
  ...
  fda-devices-pp-cli sync

  # Sync specific resources only
  fda-devices-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  fda-devices-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  fda-devices-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  fda-devices-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  fda-devices-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("fda-devices-pp-cli")
			}
			...
			// If no specific resources or "all" sentinel, sync top-level resources
			if len(resources) == 0 || (len(resources) == 1 && strings.EqualFold(resources[0], "all")) {
	...
	cmd.Flags().StringSliceVar(&resources, "source", nil, "Alias for --resources (use 'all' or comma-separated names)")
	cmd.Flags().BoolVar(&full, "full", false, "Full resync (ignore previous checkpoint)")
	cmd.Flags().StringVar(&since, "since", "", "Incremental sync duration (e.g. 7d, 24h, 1w, 30m)")
	cmd.Flags().IntVar(&concurrency, "concurrency", 4, "Number of parallel sync workers")
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/fda-devices-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"device": "/device/pma.json",
		"device-510k-json": "/device/510k.json",
		"device-classification-json": "/device/classification.json",
		"device-event-json": "/device/event.json",
		"device-recall-json": "/device/recall.json",
		"device-registrationlisting-json": "/device/registrationlisting.json",
	}
