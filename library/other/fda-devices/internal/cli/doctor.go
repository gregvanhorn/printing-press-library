...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fda-devices-pp-cli doctor
  fda-devices-pp-cli doctor --json
  fda-devices-pp-cli doctor --fail-on warn`,
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'fda-devices-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'fda-devices-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'fda-devices-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'fda-devices-pp-cli sync' to refresh."
	}
	return report
}
