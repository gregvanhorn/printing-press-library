...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".fda-devices-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FDA_DEVICES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FDA_DEVICES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fda-devices-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.fda-devices-pp-cli/feedback.jsonl.
When ` + "`FDA_DEVICES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FDA_DEVICES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fda-devices-pp-cli feedback list
  fda-devices-pp-cli feedback list --limit 5
  fda-devices-pp-cli feedback list --json`,
