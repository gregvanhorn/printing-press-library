...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".hackernews-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HACKERNEWS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HACKERNEWS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hackernews-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.hackernews-pp-cli/feedback.jsonl.
When ` + "`HACKERNEWS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HACKERNEWS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
