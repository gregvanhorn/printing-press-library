...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".movie-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MOVIE_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MOVIE_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "movie-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.movie-goat-pp-cli/feedback.jsonl.
When ` + "`MOVIE_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MOVIE_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
