...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  steam-web-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  steam-web-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"iecon-service", "ipublished-file-service", "istore-service"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  # Show status as JSON
  steam-web-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("steam-web-pp-cli")
			}
