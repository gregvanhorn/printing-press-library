...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".wikipedia-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WIKIPEDIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WIKIPEDIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "wikipedia-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.wikipedia-pp-cli/feedback.jsonl.
When ` + "`WIKIPEDIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WIKIPEDIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
