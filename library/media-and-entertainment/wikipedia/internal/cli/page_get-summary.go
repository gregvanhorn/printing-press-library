...
func newPageGetSummaryCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path = replacePathParam(path, "title", args[0])
			params := map[string]string{}
			if flagRedirect != false {
				params["redirect"] = fmt.Sprintf("%v", flagRedirect)
			}
			data, prov, err := resolveRead(c, flags, "page", false, path, params)
			if err != nil {
	...
	cmd.Flags().BoolVar(&flagRedirect, "redirect", true, "Follow redirects")
