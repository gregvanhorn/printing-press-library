...
Exit codes & warnings:
  ...
  podscan-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  podscan-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  podscan-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  podscan-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  podscan-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("podscan-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/podscan-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "episodes":
		return db.UpsertEpisodes(data)
	case "podcasts":
		return db.UpsertPodcasts(data)
	case "podcasts_episodes":
		return db.UpsertPodcastsEpisodes(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"exports": "/exports/episodes",
		"exports-podcasts": "/exports/podcasts",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "mentions", ParentTable: "alerts", ParentIDParam: "alert_id", PathTemplate: "/alerts/{alert_id}/mentions"},
	}
}
