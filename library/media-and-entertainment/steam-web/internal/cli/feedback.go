...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".steam-web-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("STEAM_WEB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("STEAM_WEB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "steam-web-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.steam-web-pp-cli/feedback.jsonl.
When ` + "`STEAM_WEB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`STEAM_WEB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
