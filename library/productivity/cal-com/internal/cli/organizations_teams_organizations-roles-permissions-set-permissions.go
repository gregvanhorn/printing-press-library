...
func newOrganizationsTeamsOrganizationsRolesPermissionsSetPermissionsCmd(flags *rootFlags) *cobra.Command {
	var bodyPermissions string
	var stdinBody bool

	cmd := &cobra.Command{
		Use:     "organizations-roles-permissions-set-permissions <orgId> <teamId> <roleId>",
		Short:   "Replace all permissions for an organization team role",
		Example: "  cal-com-pp-cli organizations teams organizations-roles-permissions-set-permissions 42 42 example-value",
	...
	cmd.Flags().StringVar(&bodyPermissions, "permissions", "", "Permissions to add (format: resource.action)")
