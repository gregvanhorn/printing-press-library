...
func newUserauditlogListPromotedCmd(flags *rootFlags) *cobra.Command {
	var flagDate string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/userauditlog.list"
			params := map[string]string{}
			if flagDate != "" {
				params["date"] = fmt.Sprintf("%v", flagDate)
			}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "userauditlog-list", false, path, params, nil)
			if err != nil {
	...
	cmd.Flags().StringVar(&flagDate, "date", "", "The date to pull audit log entries from. All activities from that date in UTC are returned.")
