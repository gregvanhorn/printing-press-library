...
In local mode: searches locally synced data only.`,
  ...
  roam-pp-cli search "error timeout"

  # Force local search only
  roam-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  roam-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  roam-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("roam-pp-cli")
			}

			db, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'roam-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "onair_event_create":
				results, err = db.SearchOnairEventCreate(query, limit)
			case "onair_event_info":
				results, err = db.SearchOnairEventInfo(query, limit)
			case "onair_event_update":
				results, err = db.SearchOnairEventUpdate(query, limit)
			case "schemas":
				results, err = db.SearchSchemas(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchOnairEventCreate(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search onair_event_create failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchOnairEventInfo(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search onair_event_info failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchOnairEventUpdate(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search onair_event_update failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSchemas(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search schemas failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/roam-pp-cli/data.db)")
