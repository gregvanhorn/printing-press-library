...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".roam-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ROAM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ROAM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "roam-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.roam-pp-cli/feedback.jsonl.
When ` + "`ROAM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ROAM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  roam-pp-cli feedback list
  roam-pp-cli feedback list --limit 5
  roam-pp-cli feedback list --json`,
