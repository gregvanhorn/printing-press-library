...
Exit codes & warnings:
  ...
  roam-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  roam-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  roam-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  roam-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  roam-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "group_create":
		return db.UpsertGroupCreate(data)
	case "groups":
		return db.UpsertGroups(data)
	case "groups_list":
		return db.UpsertGroupsList(data)
	case "item_upload":
		return db.UpsertItemUpload(data)
	case "magicast_info":
		return db.UpsertMagicastInfo(data)
	case "meetinglink_info":
		return db.UpsertMeetinglinkInfo(data)
	case "onair_event_create":
		return db.UpsertOnairEventCreate(data)
	case "onair_event_info":
		return db.UpsertOnairEventInfo(data)
	case "onair_event_update":
		return db.UpsertOnairEventUpdate(data)
	case "onair_guest_info":
		return db.UpsertOnairGuestInfo(data)
	case "onair_guest_update":
		return db.UpsertOnairGuestUpdate(data)
	case "resource_types":
		return db.UpsertResourceTypes(data)
	case "schemas":
		return db.UpsertSchemas(data)
	case "transcript_info":
		return db.UpsertTranscriptInfo(data)
	case "user_info":
		return db.UpsertUserInfo(data)
	case "user_lookup":
		return db.UpsertUserLookup(data)
	case "users":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"magicast-list": "/magicast.list",
		"meeting-list": "/meeting.list",
		"onair-event-list": "/onair.event.list",
		...
		"user-list": "/user.list",
		"userauditlog-list": "/userauditlog.list",
		"users": "/Users",
	}
