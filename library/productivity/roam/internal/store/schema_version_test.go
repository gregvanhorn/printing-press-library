...
// TestMigrate_AddsColumnsOnUpgrade_GroupCreate verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_GroupCreate(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE group_create (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(group_create)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from group_create after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_GroupsList verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_GroupsList(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE groups_list (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(groups_list)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from groups_list after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_ItemUpload verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ItemUpload(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE item_upload (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(item_upload)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from item_upload after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MagicastInfo verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MagicastInfo(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE magicast_info (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(magicast_info)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from magicast_info after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_MeetinglinkInfo verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_MeetinglinkInfo(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE meetinglink_info (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(meetinglink_info)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from meetinglink_info after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_OnairEventCreate verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_OnairEventCreate(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE onair_event_create (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(onair_event_create)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from onair_event_create after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_OnairEventInfo verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_OnairEventInfo(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE onair_event_info (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(onair_event_info)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from onair_event_info after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_OnairEventUpdate verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_OnairEventUpdate(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE onair_event_update (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(onair_event_update)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from onair_event_update after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_OnairGuestInfo verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_OnairGuestInfo(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE onair_guest_info (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(onair_guest_info)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from onair_guest_info after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_OnairGuestUpdate verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_OnairGuestUpdate(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE onair_guest_update (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(onair_guest_update)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from onair_guest_update after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_TranscriptInfo verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_TranscriptInfo(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE transcript_info (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(transcript_info)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from transcript_info after migrate", want)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_UserInfo(t *testing.T) {
	...
	if _, err := raw.Exec(`CREATE TABLE user_info (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(user_info)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from user_info after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_UserLookup verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_UserLookup(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE user_lookup (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(user_lookup)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from user_lookup after migrate", want)
		}
	}
}
