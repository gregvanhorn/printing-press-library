# Roam Printed CLI Agent Guide

This directory is a generated `roam-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
roam-pp-cli doctor --json
roam-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
roam-pp-cli which "<capability>" --json
...
```bash
roam-pp-cli <command> --help
roam-pp-cli <command> --dry-run --agent
```
