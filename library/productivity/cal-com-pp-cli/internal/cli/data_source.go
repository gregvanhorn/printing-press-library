...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/cal-com-pp-cli/internal/client"
...
func resolveRead(c *client.Client, flags *rootFlags, resourceType string, isList bool, path string, params map[string]string) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if localErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'cal-com-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func resolveLocal(resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead("cal-com-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'cal-com-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'cal-com-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'cal-com-pp-cli sync' first", resourceType)
		}
	...
	if item == nil {
		return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'cal-com-pp-cli sync' first", resourceType, id)
	}
