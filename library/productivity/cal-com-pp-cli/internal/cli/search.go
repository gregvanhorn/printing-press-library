...
In local mode: searches locally synced data only.`,
  ...
  cal-com-pp-cli search "error timeout"

  # Force local search only
  cal-com-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  cal-com-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  cal-com-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'cal-com-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
