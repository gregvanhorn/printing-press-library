...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  cal-com-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  cal-com-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"bookings", "calendars", "conferencing", "event-types", "oauth-clients", "schedules", "teams", "users", "verified-resources", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  cal-com-pp-cli workflow status

  # Show status as JSON
  cal-com-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("cal-com-pp-cli")
			}
