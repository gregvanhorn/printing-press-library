...
Exit codes & warnings:
  ...
  techmeme-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  techmeme-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  techmeme-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  techmeme-pp-cli sync --concurrency 8
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "techmeme_search":
		return db.UpsertTechmemeSearch(data)
	default:
