...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  render-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  render-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"blueprints", "disks", "env-groups", "key-value", "logs", "maintenance", "metrics", "metrics-active-connections", "metrics-bandwidth", "metrics-bandwidth-sources", "metrics-cpu-limit", "metrics-cpu-target", "metrics-disk-capacity", "metrics-disk-usage", "metrics-filters-application", "metrics-filters-http", "metrics-filters-path", "metrics-http-latency", "metrics-http-requests", "metrics-instance-count", "metrics-memory", "metrics-memory-limit", "metrics-memory-target", "metrics-replication-lag", "metrics-task-runs-completed", "metrics-task-runs-queued", "notification-settings", "owners", "postgres", "projects", "redis", "registrycredentials", "services", "task-runs", "tasks", "webhooks", "workflows", "workflowversions"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  render-pp-cli workflow status

  # Show status as JSON
  render-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("render-pp-cli")
			}
