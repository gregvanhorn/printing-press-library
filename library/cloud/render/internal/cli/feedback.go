...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".render-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RENDER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RENDER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "render-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.render-pp-cli/feedback.jsonl.
When ` + "`RENDER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RENDER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  render-pp-cli feedback list
  render-pp-cli feedback list --limit 5
  render-pp-cli feedback list --json`,
