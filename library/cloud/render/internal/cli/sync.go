...
Exit codes & warnings:
  ...
  render-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  render-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  render-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  render-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  render-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "syncs":
		return db.UpsertSyncs(data)
	case "runs":
		return db.UpsertRuns(data)
	case "snapshots":
		return db.UpsertSnapshots(data)
	case "env_groups_env_vars":
		return db.UpsertEnvGroupsEnvVars(data)
	case "env_groups_secret_files":
		return db.UpsertEnvGroupsSecretFiles(data)
	case "env_groups_services":
		return db.UpsertEnvGroupsServices(data)
	case "resources":
		return db.UpsertEnvResources(data)
	case "events":
		return db.UpsertEvents(data)
	case "key_value_connection_info":
		return db.UpsertKeyValueConnectionInfo(data)
	case "key_value_resume":
		return db.UpsertKeyValueResume(data)
	case "key_value_suspend":
		return db.UpsertKeyValueSuspend(data)
	case "logs":
	...
	case "metrics_stream":
		return db.UpsertMetricsStream(data)
	case "notification_settings":
		return db.UpsertNotificationSettings(data)
	case "organizations_audit_logs":
		return db.UpsertOrganizationsAuditLogs(data)
	case "owners_audit_logs":
		return db.UpsertOwnersAuditLogs(data)
	case "members":
		return db.UpsertMembers(data)
	case "postgres_connection_info":
		return db.UpsertPostgresConnectionInfo(data)
	case "credentials":
		return db.UpsertCredentials(data)
	case "export":
		return db.UpsertExport(data)
	case "failover":
		return db.UpsertFailover(data)
	case "recovery":
		return db.UpsertRecovery(data)
	case "postgres_restart":
		return db.UpsertPostgresRestart(data)
	case "postgres_resume":
		return db.UpsertPostgresResume(data)
	case "postgres_suspend":
		return db.UpsertPostgresSuspend(data)
	case "redis_connection_info":
		return db.UpsertRedisConnectionInfo(data)
	case "autoscaling":
		return db.UpsertAutoscaling(data)
	case "cache":
		return db.UpsertCache(data)
	case "custom_domains":
		return db.UpsertCustomDomains(data)
	case "deploys":
		return db.UpsertDeploys(data)
	case "services_env_vars":
		return db.UpsertServicesEnvVars(data)
	case "services_events":
		return db.UpsertServicesEvents(data)
	case "headers":
		return db.UpsertHeaders(data)
	case "instances":
		return db.UpsertInstances(data)
	case "jobs":
		return db.UpsertJobs(data)
	case "preview":
		return db.UpsertPreview(data)
	case "services_restart":
		return db.UpsertServicesRestart(data)
	case "services_resume":
		return db.UpsertServicesResume(data)
	case "rollback":
		return db.UpsertRollback(data)
	case "routes":
		return db.UpsertRoutes(data)
	case "scale":
		return db.UpsertScale(data)
	case "services_secret_files":
		return db.UpsertServicesSecretFiles(data)
	case "services_suspend":
		return db.UpsertServicesSuspend(data)
	case "task_runs":
		return db.UpsertTaskRuns(data)
	case "webhooks_events":
		return db.UpsertWebhooksEvents(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"key-value":                   "/key-value",
		"logs":                        "/logs/streams/resource",
		"maintenance":                 "/maintenance",
		"metrics":                     "/metrics/cpu",
		"metrics-active-connections":  "/metrics/active-connections",
		"metrics-bandwidth":           "/metrics/bandwidth",
		"metrics-bandwidth-sources":   "/metrics/bandwidth-sources",
		"metrics-cpu-limit":           "/metrics/cpu-limit",
		"metrics-cpu-target":          "/metrics/cpu-target",
		"metrics-disk-capacity":       "/metrics/disk-capacity",
		"metrics-disk-usage":          "/metrics/disk-usage",
		"metrics-filters-application": "/metrics/filters/application",
		"metrics-filters-http":        "/metrics/filters/http",
		"metrics-filters-path":        "/metrics/filters/path",
		"metrics-http-latency":        "/metrics/http-latency",
		"metrics-http-requests":       "/metrics/http-requests",
		"metrics-instance-count":      "/metrics/instance-count",
		"metrics-memory":              "/metrics/memory",
		"metrics-memory-limit":        "/metrics/memory-limit",
		"metrics-memory-target":       "/metrics/memory-target",
		"metrics-replication-lag":     "/metrics/replication-lag",
		"metrics-task-runs-completed": "/metrics/task-runs-completed",
		"metrics-task-runs-queued":    "/metrics/task-runs-queued",
		"notification-settings":       "/notification-settings/overrides",
		"owners":                      "/owners",
		"postgres":                    "/postgres",
		"projects":                    "/projects",
		"redis":                       "/redis",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "custom_domains", ParentTable: "services", ParentIDParam: "serviceId", PathTemplate: "/services/{serviceId}/custom-domains"},
		{Name: "deploys", ParentTable: "services", ParentIDParam: "serviceId", PathTemplate: "/services/{serviceId}/deploys"},
		{Name: "headers", ParentTable: "services", ParentIDParam: "serviceId", PathTemplate: "/services/{serviceId}/headers"},
		{Name: "jobs", ParentTable: "services", ParentIDParam: "serviceId", PathTemplate: "/services/{serviceId}/jobs"},
		{Name: "owners_audit_logs", ParentTable: "owners", ParentIDParam: "ownerId", PathTemplate: "/owners/{ownerId}/audit-logs"},
		{Name: "routes", ParentTable: "services", ParentIDParam: "serviceId", PathTemplate: "/services/{serviceId}/routes"},
		{Name: "services_env_vars", ParentTable: "services", ParentIDParam: "serviceId", PathTemplate: "/services/{serviceId}/env-vars"},
		{Name: "services_events", ParentTable: "services", ParentIDParam: "serviceId", PathTemplate: "/services/{serviceId}/events"},
		{Name: "services_secret_files", ParentTable: "services", ParentIDParam: "serviceId", PathTemplate: "/services/{serviceId}/secret-files"},
		{Name: "syncs", ParentTable: "blueprints", ParentIDParam: "blueprintId", PathTemplate: "/blueprints/{blueprintId}/syncs"},
		{Name: "webhooks_events", ParentTable: "webhooks", ParentIDParam: "webhookId", PathTemplate: "/webhooks/{webhookId}/events"},
	}
}
