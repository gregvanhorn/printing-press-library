...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  whoop-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  whoop-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activity", "activity-workout", "cycle", "recovery",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  whoop-pp-cli workflow status

  # Show status as JSON
  whoop-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("whoop-pp-cli")
			}
