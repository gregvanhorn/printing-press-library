...
Exit codes & warnings:
  ...
  whoop-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  whoop-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  whoop-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  whoop-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  whoop-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/whoop-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "activity_mapping":
		return db.UpsertActivityMapping(data)
	case "cycle":
		return db.UpsertCycle(data)
	case "cycle_recovery":
		return db.UpsertCycleRecovery(data)
	case "sleep":
		return db.UpsertSleep(data)
	case "partner":
		return db.UpsertPartner(data)
	case "user":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"activity": "/v2/activity/sleep",
		"activity-workout": "/v2/activity/workout",
		"cycle": "/v2/cycle",
