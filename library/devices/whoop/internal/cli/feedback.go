...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".whoop-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WHOOP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WHOOP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "whoop-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.whoop-pp-cli/feedback.jsonl.
When ` + "`WHOOP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WHOOP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  whoop-pp-cli feedback list
  whoop-pp-cli feedback list --limit 5
  whoop-pp-cli feedback list --json`,
