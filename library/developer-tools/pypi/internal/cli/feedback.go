...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pypi-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PYPI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PYPI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pypi-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pypi-pp-cli/feedback.jsonl.
When ` + "`PYPI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PYPI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
