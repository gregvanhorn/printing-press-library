...
Exit codes & warnings:
  ...
  pypi-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pypi-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  pypi-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pypi-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pypi-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pypi-pp-cli/data.db)")
