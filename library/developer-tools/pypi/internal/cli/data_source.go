...
func resolveRead(c *client.Client, flags *rootFlags, resourceType string, isList bool, path string, params map[string]string) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'pypi-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func writeThroughCache(resourceType string, data json.RawMessage) {
	db, err := store.Open(defaultDBPath("pypi-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
...
func resolveLocal(resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead("pypi-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'pypi-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'pypi-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'pypi-pp-cli sync' first", resourceType)
		}
	...
	if item == nil {
		return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'pypi-pp-cli sync' first", resourceType, id)
	}
