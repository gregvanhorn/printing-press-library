...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".firecrawl-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FIRECRAWL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FIRECRAWL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "firecrawl-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.firecrawl-pp-cli/feedback.jsonl.
When ` + "`FIRECRAWL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FIRECRAWL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
