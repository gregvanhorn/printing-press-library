...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  firecrawl-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  firecrawl-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  firecrawl-pp-cli workflow status

  # Show status as JSON
  firecrawl-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("firecrawl-pp-cli")
			}
