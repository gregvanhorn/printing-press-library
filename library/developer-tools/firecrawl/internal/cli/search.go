...
In local mode: searches locally synced data only.`,
  ...
  firecrawl-pp-cli search "error timeout"

  # Force local search only
  firecrawl-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  firecrawl-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  firecrawl-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
			query := args[0]
			// This API has a search endpoint: POST /search
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'firecrawl-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
