...
Exit codes & warnings:
  ...
  firecrawl-pp-cli sync

  # Sync specific resources only
  firecrawl-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  firecrawl-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  firecrawl-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  firecrawl-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  firecrawl-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("firecrawl-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/firecrawl-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "deep_research":
		return db.UpsertDeepResearch(data)
	case "llmstxt":
		return db.UpsertLlmstxt(data)
	case "map":
		return db.UpsertMap(data)
	case "scrape":
		return db.UpsertScrape(data)
	case "crawl":
		return db.UpsertCrawl(data)
	case "errors":
		return db.UpsertErrors(data)
	case "extract":
		return db.UpsertExtract(data)
	case "search":
