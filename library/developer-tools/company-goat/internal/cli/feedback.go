...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".company-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COMPANY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COMPANY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "company-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.company-goat-pp-cli/feedback.jsonl.
When ` + "`COMPANY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COMPANY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
