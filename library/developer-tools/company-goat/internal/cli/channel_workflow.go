...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  company-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  company-goat-pp-cli workflow archive --full`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/company-goat-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  company-goat-pp-cli workflow status

  # Show status as JSON
  company-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("company-goat-pp-cli")
			}
