...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  trigger-dev-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  trigger-dev-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"queues", "runs", "schedules", "waitpoints"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  trigger-dev-pp-cli workflow status

  # Show status as JSON
  trigger-dev-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("trigger-dev-pp-cli")
			}
