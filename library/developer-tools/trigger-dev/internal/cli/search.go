...
In local mode: searches locally synced data only.`,
  ...
  trigger-dev-pp-cli search "error timeout"

  # Force local search only
  trigger-dev-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  trigger-dev-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  trigger-dev-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'trigger-dev-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
