...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".trigger-dev-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRIGGER_DEV_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRIGGER_DEV_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "trigger-dev-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.trigger-dev-pp-cli/feedback.jsonl.
When ` + "`TRIGGER_DEV_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRIGGER_DEV_FEEDBACK_AUTO_SEND=true`" + `, the entry is
