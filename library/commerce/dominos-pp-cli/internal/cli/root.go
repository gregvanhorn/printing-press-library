...
func Execute() error {
	...
	rootCmd := &cobra.Command{
		Use:           "dominos-pp-cli",
		Short:         "Domino's Pizza CLI - order pizza, browse menus, track orders, and manage rewards from the terminal",
		SilenceUsage:  true,
		SilenceErrors: true,
		Version:       version,
	}
	rootCmd.SetVersionTemplate("dominos-pp-cli {{ .Version }}\n")
	...
	menuCmd := newMenuCmd(&flags)
	menuCmd.AddCommand(newMenuSearchCmd(&flags))
	menuCmd.AddCommand(newMenuDiffCmd(&flags))
	rootCmd.AddCommand(menuCmd)
	rootCmd.AddCommand(newOrdersCmd(&flags))
	rootCmd.AddCommand(newCartCmd(&flags))
	rootCmd.AddCommand(newDealsCmd(&flags))
	rootCmd.AddCommand(newComparePricesCmd(&flags))
