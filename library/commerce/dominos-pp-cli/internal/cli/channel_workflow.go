...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "  dominos-pp-cli workflow order --template friday-night",
	}
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  dominos-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  dominos-pp-cli workflow archive --full`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.config/dominos-pp-cli/store.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  dominos-pp-cli workflow status

  # Show status as JSON
  dominos-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("dominos-pp-cli")
			}
